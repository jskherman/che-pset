    ...
    runchart = (
        ...
        .encode(
            ...
            color=alt.Color(
                "Correct",
                scale=alt.Scale(
                    domain=["Correct", "Incorrect"], range=["green", "red"]
                ),
                legend=None,
            ),
            tooltip=[
                alt.Tooltip("QNum", title="Question No"),
                alt.Tooltip("Correct", title="Status"),
                alt.Tooltip("Streak", title="Streak"),
            ],
            shape=alt.Shape(
                "Correct",
                scale=alt.Scale(
                    domain=["Correct", "Incorrect"], range=["circle", "triangle-up"]
                ),
            ),
        )
...
with st.expander("Result Summary", expanded=True):
    ...
    with rs2:
        st.metric(
            "Accuracy", f"{round(sum(results['Correct']) / len(results) * 100, 2)}%"
        )
    with rs3:
...
incorrect_tags_list = "; ".join(
    list(set([item for sublist in incorrect["Tags"].tolist() for item in sublist]))
)
            ...
            run_tags = "; ".join(
                list(
                    set(
                        [
                            item
                            for sublist in results["Tags"].tolist()
                            for item in sublist
                        ]
                    )
                )
